  ...
  mounted() {
    ...
    const appendControls = () => {
      for (let i = 0; i < limit; i++) {
        $('.carousel__control').append(`<a class="carousel__link" href="#" data-index="${i}"></a>`)
      }
...
$width: calc(50vw - #{$page-margin-x});
$half-width: calc(50vw - #{($page-margin-x / 2)});
$neg-half-width: calc(-25vw + #{($page-margin-x / 2)});
...
.carousel__stage {
  position: absolute;
  top: 2rem;        /* $page-margin-y */
  bottom: 2rem;     /* $page-margin-y */
  left: 2rem;       /* $page-margin-x */
  right: 2rem;      /* $page-margin-x */
  margin: auto;
  transform-style: preserve-3d;
  transform: translateZ(-150px); /* $neg-half-height */
}
...
.spinner {
  ...
  margin: auto;
  transform-style: preserve-3d;
  transition: transform 0.8s; // $transition-dur 
...
.spinner--right .js-spin-fwd .spinner {
  transform: rotateX(90deg);
}

.spinner--right .js-spin-bwd .spinner {
  transform: rotateX(-90deg);
}
...
.spinner__face.js-next {
  display: block;
  transform: rotateX(90deg) translateZ(150px); // $half-height 
}
...
[data-type="iceland"] .spinner--right .content__left {
...
[data-type="china"] .spinner--right .content__left {
  background-image: none;
}
...
[data-type="usa"] .spinner--right .content__left {
  background-image: none;
}
...
[data-type="peru"] .spinner--right .content__left {
  background-image: none;
}
